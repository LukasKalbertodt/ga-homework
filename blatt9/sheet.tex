...
\begin{minipage}[h]{0.75\textwidth}
    ...
    \begin{tikzpicture}[every loop/.style={}]
        \node[node] (r) {$r_i$};
        \node[node, right=of r] (g) {$g_j$};
        \node[node, right=of g] (b) {$b_k$};
        \node[node, right=of b] (w) {$w_l$};
        \path[-] (r) edge [loop] ();
        \path[-] (g) edge [loop] ();
        \path[-] (b) edge [loop] ();
        \path[-] (w) edge [loop] ();
...
\textbf{Schritt 3: Auswählen der Kanten}
\begin{compactenum}[(i)]
\item Wähle beim Teilgraphen $G_1=(V_1,E_1)$ die Kante $e_1=\lbrace u_1,v_1 \rbrace$, mit $deg(u_1)$ ungerade, aus und markiere $u_1$ als vorderen und $v_1$ als hinteren Knoten. (Für einen Knoten $u$ mit einer Schlinge gilt: $deg(u)$ ungerade.)
\begin{compactenum}[1.]
    \item Fall: $u_1\neq v_1$\\
    Wähle im nächsten Graphen einen Knoten v
\end{compactenum}
