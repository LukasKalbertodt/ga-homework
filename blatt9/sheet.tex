...
\newcommand{\addlabel}[1]{
    \foreach \start/\end/\style/\labelstart/\labelend/\posstart/\posend in {#1}
        \draw [\style] (\start) node [label={[label distance=10pt]\posstart:\labelstart}] {} to (\end) node [label={[label distance=10pt]\posend:\labelend}] {};
}
...
Für $E_{hv}$ gilt: Für die Kanten aus $E_{hv}$, die keine Schlingen sind, existieren weitere Kanten in $E$, sodass ein Zykel entsteht mit alternierend Matching und Nicht-Matching-Kante. Eine Schlinge ist ein Zykel.

Für die Kantenmenge $E_{ou}$ (oben-unten) gilt: Sie entsteht, indem die Kanten mit den bisher nicht indizierten Knoten betrachtet und solche 4 Knoten mit der Markierung "o" (oben) versehen werden, die sich paarweise in der Färbung unterscheiden.
\newline
...
\begin{tikzpicture}[node distance=1.0cm, every loop/.style={}]
